...
from models.skim_gram_H.huffman_tree import HuffmanTree
...
class InputData:
    ...
    def get_negative_sampling(self, pos_pairs, neg_count):
        """
        为正采样对生成负采样对
        :param pos_pairs: 正采样对列表 [(w, v), ...]
        :param neg_count: 每个正采样对生成的负采样个数
        :return: 负采样对 [(w, v_neg), ...]
        """
        neg_word_pair = []

        # 构建负采样用的分布 (使用 wordId_frequency_dict)
        word_ids = np.array(list(self.wordId_frequency_dict.keys()))
        word_freqs = np.array(list(self.wordId_frequency_dict.values()), dtype=np.float32)
        # 根据论文中的建议，频率取 3/4 次方以加强高频单词
        ...
        for pair in pos_pairs:
            ...
            for _ in range(neg_count):
                while True:
                    # 按照分布随机采样一个负样本
                    v_neg = np.random.choice(word_ids, p=sampling_probs)
                    # 确保负样本不与正样本重复
                    if v_neg != pair[1]:
                        break
                neg_word_pair.append((w, v_neg))
...
def test():
    ...
    for pair in neg_pair:
        neg_word_pair.append(
            (test_data.id2word_dict[pair[0]], test_data.id2word_dict[pair[1]]))
    print(neg_word_pair)
